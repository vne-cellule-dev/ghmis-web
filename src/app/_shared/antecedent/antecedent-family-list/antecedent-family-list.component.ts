...
import { AntecedentFamily } from 'src/app/_models/antecedentFamily';
...
export class AntecedentFamilyListComponent implements OnInit {
   ...
   /**
     ...
     */
    antecedentFamilies: AntecedentFamily[];
   ...
   */
    ...
    getAntecedentFamilies() {
      ...
      this.antecedentFamilyService.findAllByPage(data).subscribe(
        res => {
          if (res) {
            this.loading = false;
  ...
  disable() {
    Swal.fire({
      title: 'Souhaitez-vous vraiment desactiver cette famille d\'antécédent?',
      text: '',
      icon: 'warning',
      ...
      if (result.value) {
        ...
        this.antecedentFamilyService.disable(this.selectedItem).subscribe(result => {
          ...
          if (result) disableItem(this.antecedentFamilies, result['id']);

          Swal.fire(
            'Desactivé!',
            'famille d\'antécédent desactiver avec succès.',
            'success'
          );
        ...
        Swal.fire(
          'Annulée',
          'la famille d\'antécédent n\'a pas été modifiée.',
          'error'
        )
      }
    })
  ...
  enable() {
    Swal.fire({
      title: 'Souhaitez-vous vraiment Activer cette famille d\'antécédent?',
      text: '',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Oui.',
      ...
      if (result.value) {
        this.antecedentFamilyService.enable(this.selectedItem).subscribe(result => {
          ...
          if (result) enableItem(this.antecedentFamilies, result['id']);

          Swal.fire(
            'Actvé!',
            'famille d\'antécédent activé avec succès.',
            'success'
          );
